  ...
  return (
    <div className="screen-anim" style={{
      ...
      <div style={{ display: 'flex', gap: 8, overflowX: 'auto', padding: '0 16px 12px', flex: 'none' }}>
        <button onClick={() => setCat('all')} style={{ border: 'none', background: 'none', padding: 0, cursor: 'pointer', flex: 'none' }}>
          <span className="pill" style={{ background: cat === 'all' ? 'var(--ink)' : '#fff', color: cat === 'all' ? '#fff' : 'var(--ink)' }}>전체</span>
        </button>
        {Object.keys(CATS).map(k => (
          <button key={k} onClick={() => setCat(k)} style={{ border: 'none', background: 'none', padding: 0, cursor: 'pointer', flex: 'none' }}>
      ...
      <div className="screen" style={{ padding: '4px 16px 0' }}>
        {!showResults ? (
          <>
            ...
            <h3 className="t-head" style={{ margin: '0 0 12px', fontSize: 16 }}>최근 기록</h3>
...
function ResultRow({ rec, onOpen }) {
  return (
    <button onClick={() => onOpen(rec.id)} className="card-flat" style={{
