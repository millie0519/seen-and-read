...
function RecordScreen({ rec: initialRec, onClose, onSaved }) {
  ...
  return (
    <div className="screen-anim" style={{ position: 'absolute', inset: 0, background: 'var(--paper)', zIndex: isEdit ? 60 : 50, display: 'flex', flexDirection: 'column' }}>
      {/* header */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '52px 18px 10px', flex: 'none' }}>
        ...
        <span className="t-display" style={{ fontSize: 22 }}>{isEdit ? '기록 수정' : '새 기록'}</span>
      ...
      <div className="screen" style={{ padding: '4px 18px 0' }}>
        ...
        {/* tags */}
        <label className="t-head" style={lbl}>태그</label>
        {tags.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
            {tags.map((tag, i) => (
              <span key={i} className="pill" style={{ background: 'var(--sky)', fontSize: 13, padding: '5px 10px' }}>
                #{tag}
                <button onClick={() => setTags(ts => ts.filter((_, j) => j !== i))} style={{ ...btnReset, marginLeft: 4 }}>
        ...
        <div className="card-flat" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderRadius: 14, marginBottom: 18, boxShadow: 'var(--shadow-sm)' }}>
        ...
        {/* quotes */}
        <label className="t-head" style={{ ...lbl, marginBottom: 8 }}>인용 <span className="muted" style={{ fontSize: 12 }}>(기억하고 싶은 문장)</span></label>
        ...
        <div className="screen" style={{ padding: '0 18px 20px' }}>
          {q.trim() ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              {!results.length && (
                <div className="muted" style={{ fontSize: 14, padding: '16px 4px' }}>
                  '{q}' 검색 결과가 없어요.{' '}
                  <button onClick={() => onPick(q)} style={{ ...btnReset, color: 'var(--coral-d)', fontFamily: 'var(--font-head)', textDecoration: 'underline' }}>'{q}' 사용</button>
                </div>
              )}
            </div>
          ) : (
