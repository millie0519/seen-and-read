  ...
  return (
    <div className="screen screen-anim" style={{ padding: '4px 16px 0' }}>
      ...
      <div style={{ display: 'flex', gap: 8, overflowX: 'auto', margin: '0 -16px', padding: '0 16px 16px' }}>
        <button onClick={() => setActive('all')} className="pill" style={{ border: 'none', padding: 0, background: 'none', cursor: 'pointer', flex: 'none' }}>
        ...
        {shelfCounts.map(({ cat, n }) => (
          <button key={cat} onClick={() => setActive(cat)} style={{ border: 'none', padding: 0, background: 'none', cursor: 'pointer', flex: 'none' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 22 }}>
        {filtered.slice(0, 3).map(rec => (
          <div key={rec.id} onClick={() => onOpen(rec.id)} style={{ cursor: 'pointer' }}>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end', overflowX: 'auto', paddingBottom: 8 }}>
            {books.map((b, i) => (
              <div key={b.id} onClick={() => onOpen(b.id)} style={{
                width: 46, height: 110 + (i % 3) * 14, background: b.cover, color: b.coverFg,
                border: '2px solid var(--ink)', borderRadius: '4px 4px 2px 2px', flex: 'none',
                transform: `rotate(${(i % 4 - 1.5) * 0.6}deg)`, position: 'relative',
                display: 'flex', alignItems: 'flex-end', padding: 6, boxSizing: 'border-box', cursor: 'pointer',
              }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
          {movies.map(m => (
            <div key={m.id} onClick={() => onOpen(m.id)} style={{
              width: '100%', height: 150, background: m.cover, color: m.coverFg, cursor: 'pointer',
      ...
      {records.length === 0 && (
        <div className="muted" style={{ textAlign: 'center', fontSize: 14, padding: '40px 0' }}>아직 기록이 없어요</div>
      )}
    </div>
  );
}
